...
def prune_detections(
    detections, predictions, threshold, number_of_detections=float("inf")
):
    """Prunes the given detected bounding boxes according to their predictions
    and returns the pruned bounding boxes and their predictions
    For threshold >= 1., all detections will be returned (i.e., no pruning is
    performed), but the list will be sorted with descendant predictions.
    ...
    """
    # sort boxes
    sorted = [(predictions[i], i) for i in range(len(predictions))]
    ...
    if threshold >= 1:
        ...
        for i, (pred, idx) in enumerate(sorted):
            for _, (_, idx_) in enumerate(pruned):
                if detections[idx].similarity(detections[idx_]) > threshold:
                    break
            else:  # this else is for the for loop
                pruned.append((pred, idx))
                if number_of_detections > 0 and len(pruned) == number_of_detections:
                    break

    # fill pruned boxes
    pruned_boxes = [detections[idx] for _, idx in pruned]
    pruned_predictions = [pred for pred, _ in pruned]
    for i in range(len(pruned_predictions)):
        pruned_predictions[i] = sorted[i][0]
    return pruned_boxes, pruned_predictions
