...
def get_same_padding(kernel_size):
    if isinstance(kernel_size, tuple):
        assert len(kernel_size) == 2, "invalid kernel size: {}".format(
            kernel_size
        )
...
class HardSwish(nn.Module):
    ...
    def forward(self, x):
        if self.inplace:
            return x.mul_(F.relu6(x + 3.0, inplace=True) / 6.0)
        else:
        ...
        if self.use_bn:
            if self.bn_before_weight:
                self.bn = nn.BatchNorm2d(
                    in_channels, affine=affine, track_running_stats=affine
                )
            else:
                self.bn = nn.BatchNorm2d(
                    out_channels, affine=affine, track_running_stats=affine
                )
        else:
            self.bn = None
        # activation
...
class ConvLayer(BasicLayer):
    def __init__(
        ...
        super(ConvLayer, self).__init__(
            in_channels, out_channels, use_bn, affine, act_func, ops_order
        )
        ...
        if self.use_bn:
            if self.bn_before_weight:
                self.bn = nn.BatchNorm1d(
                    in_features, affine=affine, track_running_stats=affine
                )
            else:
                self.bn = nn.BatchNorm1d(
                    out_features, affine=affine, track_running_stats=affine
                )
        else:
            self.bn = None
        # activation
...
class MBInvertedResBlock(nn.Module):
    def __init__(
        ...
        if mid_channels > in_channels:
            ...
            if use_bn:
                inverted_bottleneck["bn"] = nn.BatchNorm2d(
                    mid_channels, affine=affine, track_running_stats=affine
                )
            if act_func == "relu":
        ...
        if se_channels > 0:
            ...
            squeeze_excite["conv_expand"] = nn.Conv2d(
                se_channels, mid_channels, 1, 1, 0, groups=groups, bias=True
            )
        ...
        if use_bn:
            point_linear["bn"] = nn.BatchNorm2d(
                out_channels, affine=affine, track_running_stats=affine
            )
...
class TF_NAS_A(nn.Module):
    def __init__(self, out_h, out_w, feat_dim, drop_ratio=0.0):
        ...
        self.first_stem = ConvLayer(
            3, 32, kernel_size=3, stride=1, act_func="relu"
        )
        self.second_stem = MBInvertedResBlock(
            32, 32, 8, 16, kernel_size=3, stride=1, act_func="relu"
        )
        self.stage1 = nn.Sequential(
            MBInvertedResBlock(
                16, 83, 32, 24, kernel_size=3, stride=2, act_func="relu"
            ),
            MBInvertedResBlock(
                24, 128, 0, 24, kernel_size=5, stride=1, act_func="relu"
            ),
        )
        self.stage2 = nn.Sequential(
            MBInvertedResBlock(
                24, 138, 48, 40, kernel_size=3, stride=2, act_func="swish"
            ),
            MBInvertedResBlock(
                40, 297, 0, 40, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                40, 170, 80, 40, kernel_size=5, stride=1, act_func="swish"
            ),
        )
        self.stage3 = nn.Sequential(
            MBInvertedResBlock(
                40, 248, 80, 80, kernel_size=5, stride=2, act_func="swish"
            ),
            MBInvertedResBlock(
                80, 500, 0, 80, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                80, 424, 0, 80, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                80, 477, 0, 80, kernel_size=3, stride=1, act_func="swish"
            ),
        )
        self.stage4 = nn.Sequential(
            MBInvertedResBlock(
                80, 504, 160, 112, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                112, 796, 0, 112, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                112, 723, 224, 112, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                112, 555, 224, 112, kernel_size=3, stride=1, act_func="swish"
            ),
        )
        self.stage5 = nn.Sequential(
            MBInvertedResBlock(
                112, 813, 0, 192, kernel_size=3, stride=2, act_func="swish"
            ),
            MBInvertedResBlock(
                192, 1370, 0, 192, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                192, 1138, 384, 192, kernel_size=3, stride=1, act_func="swish"
            ),
            MBInvertedResBlock(
                192, 1359, 384, 192, kernel_size=3, stride=1, act_func="swish"
            ),
        )
        self.stage6 = nn.Sequential(
            MBInvertedResBlock(
                192, 1203, 384, 320, kernel_size=5, stride=1, act_func="swish"
            ),
        )
        self.feature_mix_layer = ConvLayer(
            320, 1280, kernel_size=1, stride=1, act_func="none"
        )
