...
class BackboneFactory:
    ...
    def __init__(self, backbone_type, backbone_conf_file):
        ...
        with open(backbone_conf_file) as f:
            backbone_conf = yaml.load(f, yaml.Loader)
    ...
    def get_backbone(self):
        if self.backbone_type == "MobileFaceNet":
            feat_dim = self.backbone_param[
                "feat_dim"
            ]  # dimension of the output features, e.g. 512.
            out_h = self.backbone_param[
                "out_h"
            ]  # height of the feature map before the final features.
            ...
            width = self.backbone_param[
                "width"
            ]  # width for EfficientNet, e.g. 1.0, 1.2, 1.4, ...
            depth = self.backbone_param[
                "depth"
            ]  # depth for EfficientNet, e.g. 1.0, 1.2, 1.4, ...
            image_size = self.backbone_param[
                "image_size"
            ]  # input image size, e.g. 112.
            drop_ratio = self.backbone_param["drop_ratio"]  # drop out ratio.
            out_h = self.backbone_param[
                "out_h"
            ]  # height of the feature map before the final features.
            ...
            out_w = self.backbone_param[
                "out_w"
            ]  # width of the feature map before the final feature
            ...
            stage1_modules = self.backbone_param[
                "stage1_modules"
            ]  # the number of attention modules in stage1.
            stage2_modules = self.backbone_param[
                "stage2_modules"
            ]  # the number of attention modules in stage2.
            stage3_modules = self.backbone_param[
                "stage3_modules"
            ]  # the number of attention modules in stage3.
            feat_dim = self.backbone_param[
                "feat_dim"
            ]  # dimension of the output features, e.g. 512.
            out_h = self.backbone_param[
